...
def process_psl_file(args):
    ...
    diff_site_count = len(sld_diff.index)
    sld_diff.to_csv("potential_sites.csv")
    return file_name, diff_site_count
...
def main():
    ...
    with open("different_sites_per_psl.csv", 'a', encoding="utf-8") as output:
